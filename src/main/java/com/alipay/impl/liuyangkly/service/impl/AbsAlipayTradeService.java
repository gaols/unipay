...
import com.alipay.impl.liuyangkly.model.result.*;
import com.alipay.impl.liuyangkly.service.AlipayTradeService;
import com.alipay.impl.liuyangkly.utils.Constants;
import com.alipay.impl.liuyangkly.utils.Utils;
...
abstract class AbsAlipayTradeService extends AbsAlipayService implements AlipayTradeService {
    ...
    protected AlipayMchInfo mchInfo;
    ...
    private void setTradeStatus(AlipayResponse response, TradeResult result) {
    ...
    protected AlipayTradeQueryResponse loopQueryResult(AlipayTradeQueryRequestBuilder builder) {
        ...
        for (int i = 0; i < this.mchInfo.getMaxQueryRetry(); i++) {
            Utils.sleep(this.mchInfo.getQueryDuration());
    ...
    protected void asyncCancel(final AlipayTradeCancelRequestBuilder builder) {
        final AlipayMchInfo mchInfo = this.mchInfo;
        executorService.submit(new Runnable() {
            @Override
            public void run() {
                for (int i = 0; i < mchInfo.getMaxCancelRetry(); i++) {
                    Utils.sleep(mchInfo.getCancelDuration());
    ...
    protected AlipayTradePayResponse toPayResponse(AlipayTradeQueryResponse response) {
        ...
        // 补充交易状态信息
        String msg = response.getMsg() + " tradeStatus:" + response.getTradeStatus();
    ...
    // 交易异常，或发生系统错误
    boolean tradeError(AlipayResponse response) {
