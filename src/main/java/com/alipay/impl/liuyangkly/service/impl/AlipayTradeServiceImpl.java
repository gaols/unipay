...
public class AlipayTradeServiceImpl extends AbsAlipayTradeService {
    ...
    public static class ClientBuilder {
        private final AlipayMchInfo mchInfo;
        ...
        public ClientBuilder(AlipayMchInfo mchInfo) {
            this.mchInfo = mchInfo;
        }

        public AlipayTradeServiceImpl build() {
            if (StringUtils.isEmpty(gatewayUrl)) {
                gatewayUrl = mchInfo.getOpenApiDomain(); // 与mcloudmonitor网关地址不同
            }
            if (StringUtils.isEmpty(appid)) {
                appid = mchInfo.getAppid();
            }
            if (StringUtils.isEmpty(privateKey)) {
                privateKey = mchInfo.getPrivateKey();
            }
            ...
            if (StringUtils.isEmpty(alipayPublicKey)) {
                alipayPublicKey = mchInfo.getAlipayPublicKey();
            }
            if (StringUtils.isEmpty(signType)) {
                signType = mchInfo.getSignType();
            }
    ...
    public AlipayF2FPayResult tradePay(AlipayTradePayRequestBuilder builder) {
        ...
        if (response != null && Constants.SUCCESS.equals(response.getCode())) {
            ...
            AlipayTradeQueryResponse loopQueryResponse = loopQueryResult(queryBuilder);
            ...
            AlipayTradeQueryResponse queryResponse = tradeQuery(queryBuilder);
